# AI 刷题实验统计

更新时间：2026-08-13T04:29:18Z

## 总览

| 指标 | 数量 |
|---|---:|
| 题库总数 | 4406 |
| 免费可做 | 3628 |
| 付费题 | 778 |
| 已归档完整题面 | 4406 |
| 归档中可读题面 | 3628 |
| 归档中锁定/不可用题面 | 778 |
| 已开始 | 3628 |
| 已记录本地候选 | 3626 |
| Accepted | 971 |
| 免费题仍待远程 Accepted | 2657 |
| 已 defer 的题 | 14 |
| 等待复盘的 Profile/题组合 | 0 |
| 远程试跑 | 1 |
| 正式提交 | 1000 |

## 通过质量

- 首次提交通过：952
- 第一轮内通过：971
- 首投通过占已通过题比例：98.04%
- 正式提交整体通过率：97.10%

## Profile 阶梯

| Profile | 模型 | 推理档位 | 已开始 | 本地候选 | 首次成功 | defer | 提交 | Token 覆盖 |
|---|---|---|---:|---:|---:|---:|---:|---:|
| sol-low | gpt-5.6-sol | low | 0 | 0 | 0 | 0 | 0 | 0.00% |
| sol-medium | gpt-5.6-sol | medium | 16 | 6 | 0 | 8 | 0 | 0.00% |
| sol-high | gpt-5.6-sol | high | 8 | 4 | 0 | 4 | 0 | 0.00% |
| sol-xhigh | gpt-5.6-sol | xhigh | 5 | 1 | 1 | 4 | 1 | 0.00% |
| sol-max | gpt-5.6-sol | max | 0 | 0 | 0 | 0 | 0 | 0.00% |
| sol-ultra | gpt-5.6-sol | ultra | 4 | 2 | 0 | 2 | 0 | 0.00% |
| terra-low | gpt-5.6-terra | low | 0 | 0 | 0 | 0 | 0 | 0.00% |
| terra-medium | gpt-5.6-terra | medium | 3627 | 3613 | 970 | 14 | 999 | 0.00% |

> “首次成功 Profile”表示按既定升档流程首次获得 Accepted 的档位；高档可能继承低档失败产物，
> 因此它衡量的是阶梯实验结果，不等同于各模型从空白起步的独立盲测能力。

## 免费实验集难度分布

| 难度 | 免费题 | 远程 Accepted | 覆盖率 |
|---|---:|---:|---:|
| EASY | 949 | 486 | 51.21% |
| HARD | 843 | 484 | 57.41% |
| MEDIUM | 1836 | 1 | 0.05% |

## 首次成功 Profile × 难度

- EASY：sol-xhigh: 1，terra-medium: 485
- HARD：terra-medium: 484
- MEDIUM：terra-medium: 1

## 免费题流程进度 × 难度

| 难度 | 已开始 | 已提交 | 远程 Accepted | 有失败提交 | 曾 defer | 待 Accepted |
|---|---:|---:|---:|---:|---:|---:|
| EASY | 949 | 487 | 486 | 10 | 0 | 463 |
| HARD | 843 | 487 | 484 | 13 | 14 | 359 |
| MEDIUM | 1836 | 1 | 1 | 0 | 0 | 1835 |

失败正式提交：29 次，涉及 23 题。
基础设施错误：远程试跑 0 次，正式提交 138 次；均不计入模型提交数、失败数或通过率。
当前本地代码与 Accepted 代码哈希不同：4 题；其中 4 题已另有可追溯 candidate-ready，首次 Accepted 归因保持不变。
当前本地代码与 candidate-ready 哈希漂移：0 个 Profile/题组合。

## 待升级重新解题

当前没有等待更高 Profile 重新解题的题目。

## 最高档仍未解决

- sol-ultra：2 题

## Agent 贡献

- Codex Desktop / gpt-5.6-sol: 1
- Codex Desktop / gpt-5.6-terra: 970

## Token 数据完整性

- 覆盖率：0.00%
- 规则：仅统计客户端提供且注明来源的数据；缺失值不估算
